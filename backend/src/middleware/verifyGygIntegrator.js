...
/* Diagnostico seguro para a investigacao em curso (2026-09-02): o trafego
   real da GYG que bate no alias de caminho duplo (/1/1/...) esta a falhar
   SEMPRE a autenticacao, mesmo quando o mesmo IP autentica bem no caminho
   simples momentos depois -- suspeita-se que a GYG tem duas configuracoes
   de Base URL/credenciais (possivelmente Sandbox vs Producao, ja apontado
   por Ahmed como usando a mesma Base URL) e uma delas esta dessincronizada
   dos 3 pares que aceitamos. Para confirmar sem expor segredos: grava so um
   hash curto (8 chars hex, nunca reversivel) do username fornecido -- dá
   para comparar duas falhas diferentes e ver se e sempre o MESMO valor
   errado (credencial estagnada) ou um valor diferente de cada vez (outro
   tipo de problema). Nunca grava a password, nem em hash. Remover depois
   de confirmado com a GYG qual par esta desincronizado. */
...
function verifyGygIntegrator(req, res, next) {
  ...
  if (!match) {
    console.log(`[GYG Auth Diag] sem header Basic (${header ? 'header presente mas nao Basic' : 'header ausente'})`);
  ...
  try {
    ...
    console.log('[GYG Auth Diag] base64 do header Authorization nao decodificou');
    return res.status(200).json({ errorCode: 'AUTHORIZATION_FAILURE', errorMessage: 'The provided authentication credentials are not valid.' });
  }
  ...
  const matchSandbox    = matchesPair(providedUser, providedPass, 'GYG_INTEGRATOR_SANDBOX_USERNAME',    'GYG_INTEGRATOR_SANDBOX_PASSWORD');
  const matchProduction = matchesPair(providedUser, providedPass, 'GYG_INTEGRATOR_PRODUCTION_USERNAME', 'GYG_INTEGRATOR_PRODUCTION_PASSWORD');
  ...
  const matchLegacy      = matchesPair(providedUser, providedPass, 'GYG_INTEGRATOR_USERNAME',            'GYG_INTEGRATOR_PASSWORD');
  const valido = matchSandbox || matchProduction || matchLegacy;

  if (!valido) {
    console.log(`[GYG Auth Diag] FALHOU -- userHash=${shortHash(providedUser)} userLen=${providedUser.length} passLen=${providedPass.length} path=${req.path}`);
    return res.status(200).json({ errorCode: 'AUTHORIZATION_FAILURE', errorMessage: 'The provided authentication credentials are not valid.' });
  }

  console.log(`[GYG Auth Diag] OK -- par=${matchSandbox ? 'sandbox' : matchProduction ? 'production' : 'legacy'} path=${req.path}`);
  next();
}
